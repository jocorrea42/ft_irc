...
/*   Updated: 2024/07/28 16:23:06 by jocorrea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::_nickAutentication(Client *client, std::vector<std::string> params)
{
    ...
    {
        std::string old = client->getNickName();
        client->setNickName(params[0]);
        _sendAllClient(std::string(": " + old + " NICK " + params[0] + "\r\n"));
        return ;
    }
...
void	Server::_sendAllClient(std::string msg)
{
	for (size_t i = 0; i < _clients.size(); i++)
	{
		std::cout << "sendall a:" << _clients[i].getNickName() << "\n";
		_clients[i].addOutBuffer(msg + "\r\n");
		std::cout << _clients[i].getOutBuffer() << std::endl;
	}
}
